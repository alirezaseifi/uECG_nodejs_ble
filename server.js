...
prepare_parser = function()
{
	noble.on('stateChange', function(state) {
	  if (state === 'poweredOn') {
	    noble.startScanning(); // You might want to specify service UUIDs here
	  } else {
	    noble.stopScanning();
	  }
	});
...
noble.on('stateChange', function(state) {
  if (state === 'poweredOn') {
  	console.log('Searching for uECG device via BLE');
...
noble.on('discover', function(peripheral) {
  ...
  peripheral.connect(function(err) {
    ...
    peripheral.discoverServices([], function (err, services) {
    	console.log('Waiting on getting the data');
...
//	if(message_length > maxlen) return -1; //not enough received data
	...
	for(var x = 0; x < unit_list.length; x++) //check if ID is in the list
		if(unit_list[x].ID == unit_id)
		{
			if(unit_list[x].protocol_version == 6) return interpretData(pack);
		}
	...
	if(data_points > 31)
	{
		if(data_points > 56 && data_points < 82) return interpretData(pack);
		return ;
	}
...
serial_process = function(data)
{
	...
	for(var x = 0; x < stream_length-75; x++) //25 - always less than minimum valid packet length
	{
		...
		{//we detected possible start of the packet, trying to make sense of it
			...
			res = interpretData(serial_stream.slice(ppos, ppos+message_length));
			if(res === undefined)
				;
			else
			{
...
function interpretData(data) {
  ...
  try {
    ...
    if (data.length < 20) {
	  ...
	  ecg_data.data_id = interpretedData.data_id || ecg_data.data_id;
	  ecg_data.scale = interpretedData.scale || ecg_data.scale;
	  ecg_data.ecg_values = interpretedData.ecg_values || ecg_data.ecg_values;
	  ecg_data.bpm_momentary = interpretedData.bpm_momentary || ecg_data.bpm_momentary;

	  ecg_data.acc_x = interpretedData.ax || ecg_data.acc_x;
	  ecg_data.acc_y = interpretedData.ay || ecg_data.acc_y;
	  ecg_data.acc_z = interpretedData.az || ecg_data.acc_z;
	  ecg_data.dev_BPM = interpretedData.dev_BPM || ecg_data.dev_BPM;
	  ecg_data.dev_skin = interpretedData.dev_skin || ecg_data.dev_skin;

	  ecg_data.pNN_short = interpretedData.pNN_short || ecg_data.pNN_short;
	  ecg_data.dev_SDRR = interpretedData.dev_SDRR || ecg_data.dev_SDRR;
	  ecg_data.dev_RMSSD = interpretedData.dev_RMSSD || ecg_data.dev_RMSSD;
	  ecg_data.batt = interpretedData.batt || ecg_data.batt;

	  // Assuming you might want to push the interpretedData into a queue
